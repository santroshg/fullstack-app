...
export const Email = {
    send(e, o, t, n, a, s, r, c) {
      ...
      let i = `From=${e}`; i += `&to=${o}`, i += `&Subject=${encodeURIComponent(t)}`, i += `&Body=${encodeURIComponent(n)}`, void 0 === a.token ? (i += `&Host=${a}`, i += `&Username=${s}`, i += `&Password=${r}`, i += '&Action=Send') : (i += `&SecureToken=${a.token}`, i += '&Action=SendFromStored', c = a.callback), i += `&cachebuster=${d}`, Email.ajaxPost('https://smtpjs.com/v2/smtp.aspx?', i, c);
    },
...
export const sendMail = (to, boardId, userId) => {
  ...
  Email.send('slackmailing@gmail.com',
    ...
    `<a href="${hrefUrl}">Join Process Management application</a>`,
    'smtp.gmail.com',
